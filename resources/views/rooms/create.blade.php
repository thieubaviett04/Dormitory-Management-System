...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center space-x-2 text-sm font-medium text-muted-foreground">
        ...
        <span class="text-foreground">Thêm phòng mới</span>
    </div>
        ...
        <form action="{{ route('rooms.store') }}" method="POST" class="p-6 space-y-6">
            ...
            <div class="space-y-4">
                ...
                <div class="space-y-2">
                    ...
                    <input type="number" name="capacity" id="capacity" value="{{ old('capacity', 4) }}" required min="1" placeholder="Ví dụ: 4" class="flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-sm transition-colors placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-ring">
                </div>

                <!-- Trạng thái -->
                <div class="space-y-2">
                    <label class="text-sm font-medium leading-none" for="status">Trạng thái ban đầu</label>
