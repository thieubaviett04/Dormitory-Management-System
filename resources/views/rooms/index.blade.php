...
<div class="space-y-6" x-data="{
    ...
    // Filter
    filterBuildingId: sessionStorage.getItem('rooms_filterBuildingId') || '',
    ...
    get editDetectedFloor() {
        const num = parseInt(this.selectedRoom.room_number);
        if (isNaN(num) || num < 100) return null;
        return Math.floor(num / 100);
    },
    get maxFloor() {
    ...
    get editMaxFloor() {
        const b = this.buildings.find(b => b.id == this.selectedRoom.building_id);
        return b ? b.floors : 999;
    },
    get floorValid() {
    ...
    get editFloorValid() {
        if (this.editDetectedFloor === null) return true;
        return this.editDetectedFloor >= 1 && this.editDetectedFloor <= this.editMaxFloor;
    },
    get selectedBuilding() {
        return this.buildings.find(b => b.id == this.selectedBuildingId) || null;
    },
    get editSelectedBuilding() {
        return this.buildings.find(b => b.id == this.selectedRoom.building_id) || null;
    },
    init() {
        this.buildings = {{ $buildings->map(fn($b) => ['id' => $b->id, 'name' => $b->name, 'floors' => $b->floors])->values()->toJson() }};
        this.$watch('filterBuildingId', value => {
            sessionStorage.setItem('rooms_filterBuildingId', value);
        });
    },
    openEdit(room) {
        this.selectedRoom = {
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 p-4 rounded-xl border bg-card text-card-foreground shadow-sm">
        <div class="space-y-1.5">
            ...
            <select id="filter_building" x-model="filterBuildingId" class="flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1.5 text-sm shadow-sm transition-colors focus:outline-none focus:ring-1 focus:ring-ring">
        ...
        <div class="flex items-end">
            <button @click="filterBuildingId = ''" x-show="filterBuildingId" x-cloak class="inline-flex items-center justify-center rounded-md text-sm font-medium transition-colors border border-input bg-background shadow-sm hover:bg-accent hover:text-accent-foreground h-9 px-4 w-full sm:w-auto">
        ...
        <div class="relative w-full overflow-auto">
            <table class="w-full caption-bottom text-sm">
                ...
                <tbody class="[&_tr:last-child]:border-0">
                    ...
                    <tr class="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted"
                        x-show="!filterBuildingId || '{{ $room->building_id }}' == filterBuildingId">
                                        ...
                                        <div class="flex-1 px-6 py-6 space-y-6">
                                            ...
                                            <div class="space-y-4">
                                                ...
                                                <div class="space-y-2">
                                                    ...
                                                    <select name="building_id" id="create_building_id" required
                                                ...
                                                <div class="space-y-2">
                                                    ...
                                                    <input type="text" name="room_number" id="create_room_number"
                                            ...
                                            <div class="space-y-4">
                                                ...
                                                <div class="space-y-2">
                                                    ...
                                                    <div class="mt-1.5 flex items-center gap-2 text-xs">
                                                        <template x-if="editDetectedFloor !== null && editFloorValid">
                                                            <span class="inline-flex items-center gap-1 rounded-full bg-primary/10 text-primary px-2.5 py-0.5 font-medium">
                                                                <i data-lucide="layers" class="h-3 w-3"></i>
                                                                Tầng <span x-text="editDetectedFloor"></span>
                                                            </span>
                                                        </template>
                                                        <template x-if="editDetectedFloor !== null && !editFloorValid && editSelectedBuilding">
                                                            <span class="inline-flex items-center gap-1 rounded-full bg-destructive/10 text-destructive px-2.5 py-0.5 font-medium">
                                                                <i data-lucide="alert-circle" class="h-3 w-3"></i>
                                                                Tầng <span x-text="editDetectedFloor"></span> vượt quá số tầng (<span x-text="editSelectedBuilding ? editSelectedBuilding.floors : ''"></span> tầng)
                                                            </span>
                                                        </template>
                                                        <template x-if="editDetectedFloor === null && selectedRoom.room_number !== ''">
                                                            <span class="text-muted-foreground">
                                                                Số phòng phải có ít nhất 3 chữ số (vd: 101)
                                                            </span>
                                                        </template>
