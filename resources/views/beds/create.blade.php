...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center space-x-2 text-sm font-medium text-muted-foreground">
        ...
        <span class="text-foreground">Thêm giường mới</span>
    </div>

    <div>
        <h2 class="text-2xl font-semibold tracking-tight text-foreground">Thêm giường mới</h2>
        <p class="text-sm text-muted-foreground mt-1">Chọn phòng và điền thông tin giường mới.</p>
    </div>

    <!-- Main Card -->
    <div class="rounded-xl border bg-card text-card-foreground shadow-sm">
        <form action="{{ route('beds.store') }}" method="POST" class="p-6 space-y-6">
            @csrf
            ...
            <div class="space-y-4">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="bed_number" id="bed_number" value="{{ old('bed_number') }}" required placeholder="Ví dụ: G1, G2, 01, 02..." class="flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-sm transition-colors placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-ring">
                </div>
